...
@testset "Test optimization for LinHermiteMapComponent when maxterms = nothing" begin
    ...
    C_new, error_new = TransportBasedInference2.optimize(C, X, nothing; verbose = false);

    itransform!(L, X)

    L = LinHermiteMapComponent(Xlin, Clin)

    L_new, error_Lnew = TransportBasedInference2.optimize(L, Xlin, nothing; verbose = false);
...
@testset "Test optimization for LinHermiteMapComponent when maxterms is an integer" begin
    ...
    C_new, error_new = TransportBasedInference2.optimize(C, X, 4; verbose = false);

    itransform!(L, X)

    L = LinHermiteMapComponent(Xlin, Clin)

    L_new, error_Lnew = TransportBasedInference2.optimize(L, Xlin, 4; verbose = false);
...
@testset "Test optimization for LinHermiteMapComponent when maxterms is kfold" begin
    ...
    C_new, error_new = TransportBasedInference2.optimize(C, X, "kfold"; verbose = false);

    itransform!(L, X)

    L = LinHermiteMapComponent(Xlin, Clin)

    L_new, error_Lnew = TransportBasedInference2.optimize(L, Xlin, "kfold"; verbose = false);
...
@testset "Test optimization for LinHermiteMapComponent when maxterms is split" begin
    ...
    C_new, error_new = TransportBasedInference2.optimize(C, X, "split"; verbose = false);

    itransform!(L, X)

    L = LinHermiteMapComponent(Xlin, Clin)

    L_new, error_Lnew = TransportBasedInference2.optimize(L, Xlin, "split"; verbose = false);
