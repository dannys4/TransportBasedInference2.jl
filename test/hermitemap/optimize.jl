using TransportBasedInference2: ncoeff
...
@testset "Test optimization max_terms = nothing without/with QR" begin
    ...
    C_noqr, error_noqr = TransportBasedInference2.optimize(C, X, nothing; withqr = false, verbose = false);
    C_qr, error_qr = TransportBasedInference2.optimize(C, X, nothing; withqr = true, verbose = false);
...
@testset "Test optimization max_terms is an integer without/with QR" begin
    ...
    C_noqr, error_noqr = TransportBasedInference2.optimize(C, X, 4; withqr = false, verbose = false);
    C_qr, error_qr = TransportBasedInference2.optimize(C, X, 4; withqr = true, verbose = false);
...
@testset "Test optimization max_terms is kfold without/with qr" begin
    ...
    C_new_noqr, error_new_noqr = TransportBasedInference2.optimize(C, X, "kfold"; withqr = false)
    C_new_qr, error_new_qr = TransportBasedInference2.optimize(C, X, "kfold"; withqr = true)
...
@testset "Test optimization max_terms is split with/without QR" begin
   ...
   C_opt_qr, error_opt_qr = greedyfit(m, Nx, X_train, X_valid, max_iter;
    ...
    C_new_noqr, error_new_noqr = TransportBasedInference2.optimize(C, X, "split"; withqr = false)

    C_new_qr, error_new_qr = TransportBasedInference2.optimize(C, X, "split"; withqr = true)
